...
class Pot:
    ...
    def add_chips(self, player: Player, n_chips: int):
        ...
        print(f'[DEBUG] Added {n_chips} chips for player {player.name}, total pot now: {self.total()}')
        print(f'[DEBUG] Current _pot contents: {dict(self._pot)}')

    def reset(self):
        """Reset the pot."""
        print(f'[DEBUG] Resetting pot, current contents: {dict(self._pot)}')
        self._pot.clear()
        print(f'[DEBUG] Pot cleared, new contents: {dict(self._pot)}')
